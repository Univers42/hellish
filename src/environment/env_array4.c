...
/* Element `idx` of `old` replaced by the `n` strings in `elems`, everything
** after it RENUMBERED.  zsh's `a[i]=(x y)` -- and with n == 0, `a[i]=()`,
** which is how a plugin pops a stack: the element goes and the array closes
** up behind it.
**
...
**
** Assigning past the end PADS with empty elements, which is zsh's own rule:
** `a=(1 2 3); a[9]=(x)` gives nine elements, five of them empty.
*/
